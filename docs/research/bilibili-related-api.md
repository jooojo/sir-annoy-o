...
严格 top 1 的选择方式应是：验证 `code == 0` 后只检查 `data.first`；仅当它的 `bvid` 非空且不是当前视频时才接受，否则本次推荐为空。正常响应中 `data[0]` 已满足这一点；不要静默跳到 `data[1]`，因为那已经改变了“Bilibili top 1”的产品定义。若 `data` 为空或第一项无效，则“下一个”为空，不能复用旧推荐冒充本次结果。
...
2. 取实时响应的第一个有效推荐项并映射为 `VideoSearchResult`；
3. 把它作为“漫游”的下一首；
4. 当前音频推进后再次请求，以新的当前项刷新下一首。
